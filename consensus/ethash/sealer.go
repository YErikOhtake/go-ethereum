...
func (ethash *Ethash) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}, force bool) error {
	...
	if ethash.shared != nil {
		return ethash.shared.Seal(chain, block, results, stop, force)
	}
	...
	go func() {
		...
		case <-ethash.update:
			...
			close(abort)
			if err := ethash.Seal(chain, block, results, stop, force); err != nil {
